...
[네트워크 요구사항]
- 가장 간단·확실: 회원님 본인 컴퓨터에서 실행한다(로컬 `claude` CLI, 또는 Remote Control
  브리지 세션). 이 경우 회원님 컴퓨터의 네트워크·브라우저 로그인을 그대로 쓰므로
  무료글·유료 구독글 모두 자동 수집된다. (권장 실행 환경)
- 반대로 격리된 클라우드 VM 세션(claude.ai/code의 기본 Trusted 네트워크)에서는
  substack.com이 차단되어 자동 수집이 안 된다. 이때만 아래 대안이 필요하다.
...
[유료·구독자 전용 글 자동 수집 — 선택 (Pro/Max 플랜)]
- 유료글은 로그인 없이는 본문을 받을 수 없다. 세션에 로그인 쿠키를 안전하게 넘기려면
  클라우드 환경의 "API 자격증명(API credentials)"에 Substack 세션 쿠키를 등록한다.
  1. 브라우저(로그인 상태) → substack.com 쿠키에서 substack.sid 값 복사
  2. claude.ai/code 환경설정 → API credentials 추가:
       Custom headers → Name=Cookie, Prefix=(비움), Value=substack.sid=<복사한값>
       Allowed websites → *.substack.com (+ 구독하는 커스텀 도메인 퍼블도 한 줄씩)
- 자격증명에 등록된 호스트는 네트워크 차단이 우회되고, 프록시가 쿠키를 자동 주입한다.
- 자격증명이 붙는 호스트는 반드시 curl(에이전트 프록시 HTTPS_PROXY 경유)로 가져온다.
  WebFetch는 이 주입 자격증명을 태우지 못할 수 있으므로 유료 호스트엔 curl을 쓴다.
- 한계·주의:
  - 쿠키는 만료·로테이션되므로 끊기면 자격증명 값 갱신 필요(취약).
  - 커스텀 도메인 퍼블(예: newsletter.doomberg.com)은 도메인별 쿠키라 substack.sid 하나로
    커버 안 될 수 있다 → 해당 도메인 쿠키를 별도 등록하거나 본문 붙여넣기로 처리.
  - 쿠키 값은 절대 repo에 커밋하지 않는다(환경 자격증명 저장소에만 보관).
  - 본인 유료 구독을 개인용으로 읽는 정당한 용도. 다만 Substack 공식 지원 흐름은 아니다.
